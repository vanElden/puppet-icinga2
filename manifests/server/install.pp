  ...
  case $server_db_type {
    ...
    'pgsql': {
      ...
      exec { 'postgres_schema_load':
        user    => 'root',
        path    => '/usr/bin:/usr/sbin:/bin/:/sbin',
        command => "export PGPASSWORD='\\''${db_password}'\\'' && psql -U ${db_user} -h ${db_host} -d ${db_name} < ${server_db_schema_path} && export PGPASSWORD='' && touch /etc/icinga2/postgres_schema_loaded.txt",
